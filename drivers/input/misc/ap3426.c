...
/*Added by zhanghaibin for setting,20141119*/
...
static int ap3426_init_client(struct ap3426_data *data)
{
    //struct ap3426_data *data = i2c_get_clientdata(client);
    int i;
    ...
    i2c_smbus_write_byte_data(data->client, 0x1A, 0);
    i2c_smbus_write_byte_data(data->client, 0x1B, 0);
    i2c_smbus_write_byte_data(data->client, 0x1C, 0xFF);
    i2c_smbus_write_byte_data(data->client, 0x1D, 0xFF);
		/*psensor high low thread*/
    #if 0
    i2c_smbus_write_byte_data(client, 0x2A, 0x90);//20
    i2c_smbus_write_byte_data(client, 0x2B, 0x01);
    i2c_smbus_write_byte_data(client, 0x2C, 0xf4);//50
    ...
    for (i = 0; i < AP3426_NUM_CACHABLE_REGS; i++) {
	int v = i2c_smbus_read_byte_data(data->client, reg_array[i]);
	if (v < 0)
	    return -ENODEV;
	data->reg_cache[i] = v;
    }
    /* set defaults */
    ap3426_set_range(data->client, AP3426_ALS_RANGE_0);
    ap3426_set_mode(data->client, AP3426_SYS_DEV_DOWN);
    //disable als interrupt mode,zhanghaibin,20150112
    i2c_smbus_write_byte_data(data->client, 0x02, 0x80);
    ...
    i2c_smbus_write_byte_data(data->client, 0x25, 0x08);
...
static int ap3426_parse_dt(struct device *dev, struct ap3426_data *pdata)
{
	...
	pdata->int_pin = of_get_named_gpio_flags(dt, "di,irq-gpio",
	                            0, &pdata->irq_flags);
	if (pdata->int_pin < 0)
	{
